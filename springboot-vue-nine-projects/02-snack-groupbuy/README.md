...
## 已实现
商品与团购活动、事务型下单、库存/成团数量扣减、订单查询。公共基础设施包含持久化 Bearer Token 登录、USER/ADMIN RBAC、参数校验、统一异常、H2/MySQL 双数据库配置、初始化演示数据。

## 启动
1. `cd backend && SPRING_PROFILES_ACTIVE=dev mvn spring-boot:run`（默认 H2，端口 8102）
2. `cd frontend && npm install && npm run dev`（端口 5102）
3. 浏览器打开 `http://localhost:5102`。
