...
class CoreWorkflowTests {
  ...
  @Test void coreWorkflowAndInvariant() throws Exception {
    var h="Bearer "+token();
    mvc.perform(post("/api/memberships/apply").header("Authorization",h).contentType(MediaType.APPLICATION_JSON).content("{\"clubId\":1,\"motivation\":\"test\"}")).andExpect(status().isOk());
    mvc.perform(post("/api/memberships/apply").header("Authorization",h).contentType(MediaType.APPLICATION_JSON).content("{\"clubId\":1,\"motivation\":\"again\"}")).andExpect(status().is(400));
  }
}
