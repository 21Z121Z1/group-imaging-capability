...
## 已实现
馆藏、副本可用数、借阅、重复借阅阻止、归还回补。公共基础设施包含持久化 Bearer Token 登录、USER/ADMIN RBAC、参数校验、统一异常、H2/MySQL 双数据库配置、初始化演示数据。

## 启动
1. `cd backend && SPRING_PROFILES_ACTIVE=dev mvn spring-boot:run`（默认 H2，端口 8108）
2. `cd frontend && npm install && npm run dev`（端口 5108）
3. 浏览器打开 `http://localhost:5108`。
