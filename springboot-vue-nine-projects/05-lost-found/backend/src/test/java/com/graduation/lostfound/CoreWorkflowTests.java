...
class CoreWorkflowTests {
  ...
  @Test void coreWorkflowAndInvariant() throws Exception {
    var h="Bearer "+token();
    mvc.perform(post("/api/claims").header("Authorization",h).contentType(MediaType.APPLICATION_JSON).content("{\"postId\":1,\"proof\":\"detail\"}")).andExpect(status().isOk());
    mvc.perform(post("/api/claims").header("Authorization",h).contentType(MediaType.APPLICATION_JSON).content("{\"postId\":1,\"proof\":\"duplicate\"}")).andExpect(status().is(400));
  }
}
