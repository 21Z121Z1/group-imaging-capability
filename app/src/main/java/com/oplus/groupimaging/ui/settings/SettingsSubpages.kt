...
data class ScopeExplanationUiState(
    val title: String = "索引与筛选口径",
    val subtitle: String = "统计、RAW、Live 与授权范围",
    val summaryItems: List<Pair<String, String>> = listOf(
        "照片统计" to "默认按 CaptureSession 统计，一次拍摄只计入一个主图记录。",
        "RAW 规则" to "只有和主图严格配对的 RAW 伴随项会进入 RAW 规则和移动预览。",
        "Live 判定" to "读取 EXIF 与 Oplus comment，无法确认时不会标记为 Live。",
        "授权范围" to "Android Selected Photos Access 下，索引只代表当前授权给本应用的照片范围。",
    ),
) : UiState

@HiltViewModel
class ScopeExplanationViewModel @Inject constructor() :
    BaseViewModel<ScopeExplanationUiState, UiAction, UiEffect>(ScopeExplanationUiState()) {
    override fun onAction(action: UiAction) = Unit
}

@Composable
fun ScopeExplanationRoute(
    contentPadding: PaddingValues,
    viewModel: ScopeExplanationViewModel = hiltViewModel(),
) {
    val state by viewModel.uiState.collectAsStateWithLifecycle()
    ScopeExplanationScreen(
        state = state,
        contentPadding = contentPadding,
    )
}
...
@Composable
fun ScopeExplanationScreen(
    state: ScopeExplanationUiState,
    contentPadding: PaddingValues,
) {
    LazyColumn(
        modifier = Modifier
            .fillMaxSize()
            .padding(contentPadding)
            .testTag(TestTags.Screen.SCOPE_EXPLANATION),
        contentPadding = PaddingValues(16.dp),
        verticalArrangement = Arrangement.spacedBy(16.dp),
    ) {
        item { AppTopBar(state.title, state.subtitle) }
        item {
            SectionContainer("统计规则") {
                state.summaryItems.forEach { (title, value) ->
